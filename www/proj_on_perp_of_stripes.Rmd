...
NOTE: there was an error in the original as indicated by the strikethroughs below.

This note records an easy way to project an image on the orthogonal complement of the stripe space. Recall that, if `img` represents an image in memory, its rows will span the x direction and its columns the y direction when displayed. The algorithm consists of ~~two steps:~~ the following step:
...
# ERROR in original: Add projection on normalized constant image
